...
output "vm_zones" {
  ...
  value       = zipmap(azurerm_virtual_machine.vm-linux.*.id, azurerm_virtual_machine.vm-linux.*.zones)
}

output "vm_identity" {
  description = "map with key `Virtual Machine Id`, value `list of identity` created for the Virtual Machine."
  value       = zipmap(azurerm_virtual_machine.vm-linux.*.id, azurerm_virtual_machine.vm-linux.*.identity)
}
