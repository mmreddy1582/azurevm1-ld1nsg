...
###key path need to be updated once we start the pipeline 
variable "ssh_key" {
...
variable "private_ip_allocation_method" {
  type        = string 
  description = "specify the method of allocation of IP address"
  default     = "Dynamic"
}
...
variable "vm_ipaddress" {
  description = "specify the static IP address for the VM"
  type        = string 
  default     = ""
}
